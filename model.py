...
class PixelCNN(nn.Module):
    def __init__(self, nr_resnet=5, nr_filters=80, nr_logistic_mix=10,
        ...
        # Increase depth in important layers for better representation
        down_nr_resnet = [nr_resnet] + [nr_resnet + 2] * 2  # Increased from +1 to +2
        self.down_layers = nn.ModuleList([PixelCNNLayer_down(down_nr_resnet[i], nr_filters,
                                            self.resnet_nonlinearity) for i in range(3)])
        self.up_layers = nn.ModuleList([PixelCNNLayer_up(nr_resnet, nr_filters,
                                            self.resnet_nonlinearity) for _ in range(3)])
        
        # Use same number of filters but improved initialization
        ...
        # Improved conditioning with larger embedding size
    ...
    def forward(self, x, labels, sample=False):
        ...
        for i in range(3):
            ...
            # Apply attention at middle scale
            if i == 1:
        ...
        for i in range(3):
            ...
            # Apply conditional normalization after each layer
            u = self.cond_norm(u, label_embeddings)
            ul = self.cond_norm(ul, label_embeddings)

        # Apply condition and refine
